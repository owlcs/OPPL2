...
public class OPPLScriptExtendedParserTest extends OPPLScriptTypesParserTest {
	...
	public void testOld21() {
		this
				.tester("?x:CLASS, ?y:CLASS=MATCH(\"[Pizza]+\"+?x.RENDERING) SELECT ?y subClassOf Food, ?x subClassOf Thing WHERE ?y MATCH(\"[Piza]+\"+?x.RENDERING) BEGIN ADD ?y subClassOf Thing END;");
	}
