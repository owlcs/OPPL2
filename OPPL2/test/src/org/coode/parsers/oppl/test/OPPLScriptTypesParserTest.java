...
public class OPPLScriptTypesParserTest extends TestCase {
	...
	static {
		try {
			...
			TEST_ONTOLOGY = ONTOLOGY_MANAGER.loadOntology(IRI
					.create(OPPLScriptParserTest.class.getResource("test.owl")
							.toURI()));
	...
	public void testSubClassQueryNAryAxiomVariableValues() {
		String query = "?x:CLASS SELECT disjointWith set(?x.VALUES, Thing) BEGIN ADD ?x subClassOf Thing END;";
		OPPLSyntaxTree parsed = this.parse(query, PIZZA_ONTOLOGY);
		System.out.println(parsed.toStringTree());
		assertNotNull(parsed);
		assertNotNull(parsed.getOPPLContent());
