...
public abstract class AbstractTestCase extends TestCase {
			...
			try {
				...
				if (resource != null) {
					...
					o = this.ontologyManager.contains(iri) ? this.ontologyManager
							.getOntology(iri) : this.ontologyManager
							.loadOntology(iri);
				} else {
	...
	protected void execute(OPPLScript script, OWLOntology ontology, int expected) {
		try {
			...
			assertTrue("Query " + script.toString() + "\n" + " Actual "
					+ matches.size() + " Expected " + expected + "\n"
					+ resultWriter.toString(), matches.size() == expected);
		} catch (Exception e) {
			this.log(e);
		}
	}
	...
	protected OPPLScript parse(String script, String ontology,
		...
		try {
			OPPLParser parser = new ParserFactory(this.ontologyManager,
					this.getOntology(ontology), reasoner)
