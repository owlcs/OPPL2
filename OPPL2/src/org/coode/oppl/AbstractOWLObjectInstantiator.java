...
abstract class AbstractOWLObjectInstantiator implements
	...
	public OWLClassExpression visit(OWLClass desc) {
		...
		if (this.getParameters().getConstraintSystem().isVariable(desc)) {
			Variable<?> variable = this.getParameters().getConstraintSystem()
					.getVariable(desc.getIRI());
			OWLClassExpression assignmentValue = (OWLClassExpression) this
					.getParameters().getBindingNode()
	...
	public OWLObject visit(OWLDataProperty property) {
		...
		if (this.getParameters().getConstraintSystem().isVariable(property)) {
			...
			OWLDataProperty assignmentValue = (OWLDataProperty) this
					.getParameters().getBindingNode()
					.getAssignmentValue(variable, this.getParameters());
	...
	public OWLObject visit(OWLDisjointClassesAxiom axiom) {
		...
		if (OPPLOWLDisjointClassesAxiom.class
			...
			InlineSet<OWLClassExpression> inlineSet = ((OPPLOWLDisjointClassesAxiom) axiom)
					.getInlineSet();
			instatiatedDescriptions.addAll(inlineSet.compute(this
					.getParameters()));
		} else {
	...
	public OWLObject visit(OWLDisjointUnionAxiom axiom) {
		...
		Set<OWLClassExpression> instantiatedDescriptions = axiom
				.getClassExpressions();
		for (OWLClassExpression description : descriptions) {
	...
	public OWLObject visit(OWLNamedIndividual individual) {
		...
		if (this.getParameters().getConstraintSystem().isVariable(individual)) {
			...
			OWLNamedIndividual assignmentValue = (OWLNamedIndividual) this
					.getParameters().getBindingNode()
					.getAssignmentValue(variable, this.getParameters());
	...
	public OWLObject visit(OWLNegativeObjectPropertyAssertionAxiom axiom) {
		...
		return this
				...
				.getOWLNegativeObjectPropertyAssertionAxiom(
						instantiatedProperty, instantiatedSubject,
						instantiatedObject);
	}
	...
	public OWLObject visit(OWLObjectProperty property) {
		...
		if (this.getParameters().getConstraintSystem().isVariable(property)) {
			Variable<?> variable = this.getParameters().getConstraintSystem()
					.getVariable(property.getIRI());
			OWLObjectProperty assignmentValue = (OWLObjectProperty) this
					.getParameters().getBindingNode()
					.getAssignmentValue(variable, this.getParameters());
			toReturn = assignmentValue == null ? property : assignmentValue;
		}
	...
	public OWLObject visit(OWLSameIndividualAxiom axiom) {
		...
		return this.getParameters().getConstraintSystem().getOntologyManager()
				.getOWLDataFactory()
				.getOWLSameIndividualAxiom(instantiatedIndividuals);
	}
	...
	public OWLObject visit(OWLAnnotation annotation) {
		OWLDataFactory dataFactory = this.getParameters().getConstraintSystem()
				.getOntologyManager().getOWLDataFactory();
		return dataFactory.getOWLAnnotation((OWLAnnotationProperty) annotation
	...
	public OWLObject visit(OWLAnnotationProperty property) {
		...
		if (this.getParameters().getConstraintSystem().isVariable(property)) {
			Variable<?> variable = this.getParameters().getConstraintSystem()
					.getVariable(property.getIRI());
			OWLAnnotationProperty assignmentValue = (OWLAnnotationProperty) this
					.getParameters().getBindingNode()
					.getAssignmentValue(variable, this.getParameters());
			toReturn = assignmentValue == null ? property : assignmentValue;
		}
