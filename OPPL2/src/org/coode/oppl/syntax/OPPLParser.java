...
public class OPPLParser/*@bgen(jjtree)*/implements OPPLParserTreeConstants,
		OPPLParserConstants {/*@bgen(jjtree)*/
	...
	public OPPLAbstractFactory getOPPLFactory() {
		if (opplFactory == null) {
			opplFactory = new OPPLFactory(owlOntologyManager, ontology,
					reasoner);
		}
		return opplFactory;
	}
	...
	public OPPLParser(java.io.InputStream in, OWLOntologyManager m,
	...
	public void ReInit(Reader reader, OWLOntologyManager m, OWLOntology o,
			OWLReasoner r) {
		ReInit(reader);
		owlOntologyManager = m;
		ontology = o;
	...
	public OPPLParser(Reader reader, OWLOntologyManager m, OWLOntology o,
			OWLReasoner r) {
		this(reader);
		owlOntologyManager = m;
		ontology = o;
	...
	private ParseException unexpectedException() {
		return new ParseException("Unexpected condition at line "
				+ this.token.beginLine + " column " + this.token.beginColumn
				+ ": " + this.token.image);
	}
	...
	final public OPPLScript parse(ConstraintSystem cs) throws ParseException {
		...
		if (constraintSystem == null) {
			constraintSystem = getOPPLFactory().createConstraintSystem();
		}
		try {
			...
			jjtree.closeNodeScope(jjtn000, true);
			jjtc000 = false;
			OPPLScript toReturn = getOPPLFactory().buildOPPLScript(
					constraintSystem, variables, opplQuery, actions);
			if (validator != null && !validator.accept(toReturn)) {
				{
					if (true)
						throw new ParseException("Failed validation "
	...
	final public List<Variable> VariableDeclaration(
			ConstraintSystem constraintSystem) throws ParseException {
	...
	final public List<OWLAxiomChange> Actions(ConstraintSystem constraintSystem)
	...
	final public Variable VariableDefinition(ConstraintSystem constraintSystem)
			throws ParseException {
		...
		try {
			if (jj_2_1(4)) {
				...
				variableType = VariableType
						.valueOfIgnoreCase(variableTypeToken.image);
				jj_consume_token(62);
				...
				try {
					variableType = VariableType
							.valueOfIgnoreCase(variableTypeToken.image);
					v = constraintSystem.createVariable(
							variableNameToken.image, variableType);
				...
				completions.clear();
				completions.addAll(EnumSet
						.allOf(VariableScopes.Direction.class));
				direction = direction();
				...
				jjtree.closeNodeScope(jjtn000, true);
				jjtc000 = false;
				v.setVariableScope(variableScope, getOPPLFactory()
				...
				switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
					case IDENTIFIER:
						...
						try {
							...
							{
								if (true)
									return constraintSystem.createVariable(
											variableNameToken.image,
											variableType);
							}
						} catch (OPPLException e) {
							{
	...
	final public Variable opplFunction(String name, VariableType type,
			ConstraintSystem constraintSystem) throws ParseException {
		...
		switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
			case MATCH:
				...
				regexpVariable = regexpOperation(name, type, expression,
						constraintSystem);
				{
					if (true)
			...
			case CREATE:
				...
				{
					if (true)
						return constraintSystem.createStringGeneratedVariable(
								name, type, value);
				}
				break;
			...
			default:
				jj_la1[6] = jj_gen;
				expressionVariable = expressionGeneratedVariable(name, type,
						constraintSystem);
				{
					if (true)
	...
	SingleValueGeneratedVariable expressionGeneratedVariable(String name,
			VariableType type, ConstraintSystem constraintSystem)
			throws ParseException {
		...
		try {
			...
			try {
				return Utils.parseVariableExpressionGeneratedVariable(name,
						type, expression, constraintSystem);
			} catch (ParserException e) {
				Exception temp = Utils.buildException(beginningSubSection,
						beginningSubSectionLine, e);
	...
	Variable regexpOperation(String name, VariableType type,
			SingleValueGeneratedValue<String> expression,
			ConstraintSystem constraintSystem) throws ParseException {
		...
		try {
			...
			//}
			try {
				return Utils.parseRegexpGeneratedVariable(name, type,
						expression, constraintSystem);
			} catch (ParserException e) {
				Exception temp = Utils.buildException(beginningSubSection,
						beginningSubSectionLine, e);
				throw new ParseException(temp.getMessage(), temp);
	...
	final public AbstractCollectionGeneratedValue<? extends OWLObject> set(
			VariableType type, ConstraintSystem constraintSystem)
			throws ParseException {
		...
		if (v != null) {
			if (type == v.getType()) {
				...
				{
					if (true)
						return AttributeFactory.getCollectionGeneratedValue(
								attribute, v, constraintSystem);
				}
			} else {
				{
					if (true)
						throw new ParseException("Type mismatch for variable "
								+ v.getName() + ": type " + type
								+ " needed instead of the actual "
								+ v.getType());
				}
			}
		} else {
	...
	final public SingleValueGeneratedValue<String> stringOperation(
			ConstraintSystem constraintSystem, boolean spacesAdded)
			throws ParseException {
	...
	final public SingleValueGeneratedValue<String> stringConcat(
			ConstraintSystem constraintSystem, boolean spacesAdded)
			throws ParseException {
	...
	final public SingleValueGeneratedValue generatedValue(
			ConstraintSystem constraintSystem, boolean spacesAdded)
			throws ParseException {
		String valueToken;
		Token variableNameToken;
			...
			default:
				...
				if (jj_2_3(4)) {
					...
					if (v != null) {
						try {
							Attribute attribute = Attribute
									.valueOf(attributeString);
							{
								if (true)
									return AttributeFactory
											.getVariableGeneratedValue(
													attribute, v,
													constraintSystem);
							}
					...
					if (v != null) {
						...
						if (bindings.size() > 0) {
							VariableIndexGeneratedValue toReturn = new VariableIndexGeneratedValue(
									gv, index, bindings);
							{
								if (true)
									return toReturn;
						...
						}
						{
							if (true)
								throw new ParseException(
										"No matches found for variable: " + v);
						}
					} else {
						{
							if (true)
	...
	VariableScope typeScope(VariableScopes.Direction direction,
			VariableType variableType, ConstraintSystem constraintSystem)
			throws ParseException {
	...
	VariableScope parseScope(VariableScopes.Direction direction,
		...
		try {
			...
			try {
				...
				throw Utils.buildException(beginningSubSection,
						beginningSubSectionLine, e);
			}
	...
	final public AbstractConstraint Constraint(ConstraintSystem constraintSystem)
			throws ParseException {
		...
		try {
			...
			if (variable == null) {
				{
					if (true)
						throw new ParseException("Undeclared variable "
								+ t.image);
				}
			}
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
				case MATCH:
					jj_consume_token(MATCH);
					jj_consume_token(OPENPARENTHESYS);
					regexpConstraint = matchConstraint(variable,
							constraintSystem, stringOperation(constraintSystem,
									true));
					jj_consume_token(CLOSEDPARENTHESYS);
					jjtree.closeNodeScope(jjtn000, true);
					jjtc000 = false;
				...
				case NEQ:
					...
					{
						if (true)
							return new InequalityConstraint(variable,
				...
				case IN:
					...
					{
						if (true)
							return new InCollectionConstraint(variable,
	...
	InCollectionRegExpConstraint matchConstraint(Variable variable,
			ConstraintSystem cs, SingleValueGeneratedValue expression)
			throws ParseException {
		...
		try {
			...
			try {
				return new InCollectionRegExpConstraint(variable, expression,
						cs);
			} catch (PatternSyntaxException e) {
				Exception temp = Utils.buildException(expression,
						beginningSubSection, beginningSubSectionLine, e);
				throw new ParseException(temp.getMessage(), temp);
			}/*@bgen(jjtree)*/
		} finally {
			if (jjtc000) {
	...
	OWLObject OWLExpression(Variable v, ConstraintSystem constraintSystem)
		...
		try {
			...
			while (true) {
				t = getToken(1);
				if (t.kind == COMMA || t.kind == CLOSEDCURLYBRACKETS
						|| t.kind == SEMICOLON || t.kind == BEGIN
						|| t.kind == END || t.kind == WHERE || t.kind == EOF) {
					break;
				} else {
	...
	final public OPPLQuery query(ConstraintSystem constraintSystem)
			throws ParseException {
		/*@bgen(jjtree) query */
	...
	final public void queryAtom(OPPLQuery query,
			ConstraintSystem constraintSystem) throws ParseException {
		OWLAxiom axiom;
		switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
			case ASSERTED:
	...
	final public OWLAxiomChange Action(ConstraintSystem constraintSystem)
			throws ParseException {
		/*@bgen(jjtree) Action */
		...
		try {
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
				case ADD:
					...
					{
						if (true)
							return new AddAxiom(constraintSystem.getOntology(),
									axiom);
					}
					break;
				case REMOVE:
					...
					{
						if (true)
							return new RemoveAxiom(constraintSystem
									.getOntology(), axiom);
					}
					break;
				default:
	...
	OWLAxiom Clause(ConstraintSystem constraintSystem) throws ParseException {
		...
		try {
			...
			while (true) {
				...
				if (t.kind == COMMA || t.kind == SEMICOLON || t.kind == BEGIN
						|| t.kind == END || t.kind == WHERE || t.kind == EOF) {
					break;
				} else {
					manSyntaxString += t.image + " ";
			...
			try {
				...
				Exception ex = Utils.buildException(beginningSubSection,
						beginningSubSectionLine, e);
				throw new ParseException("OPPLParser.Clause() \u005c""
						+ manSyntaxString + "\u005c"\u005ct" + ex.getMessage(),
						ex);
			}
			return toReturn;/*@bgen(jjtree)*/
		} finally {
	...
	private boolean jj_3_4() {
		...
		if (jj_scan_token(63))
			return true;
		if (jj_scan_token(GROUPSVARIABLEATTRIBUTE))
			return true;
		if (jj_scan_token(OPENPARENTHESYS))
			return true;
		return false;
	}
	...
	private boolean jj_3_3() {
		...
		if (jj_scan_token(63))
			return true;
		if (jj_scan_token(RENDERINGVARIABLEATTRIBUTE))
			return true;
		return false;
	}
	...
	private boolean jj_3_2() {
		...
		if (jj_scan_token(VARIABLETYPE))
			return true;
		if (jj_scan_token(OPENSQUAREBRACKETS))
			return true;
		return false;
	}
	...
	private static void jj_la1_init_0() {
		jj_la1_0 = new int[] { 0x1000, 0x200000, 0x4000000, 0x10000000,
				0x10000000, 0x1000, 0xf0000, 0x0, 0x0, 0x0, 0x10000000,
				0x80000, 0x10000000, 0x10000000, 0x400000, 0x800000, 0x3000000, };
	}

	private static void jj_la1_init_1() {
		jj_la1_1 = new int[] { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4,
				0x240800, 0x0, 0x300, 0x0, 0x0, 0x0, 0x0, 0x0, };
	}

	private static void jj_la1_init_2() {
		jj_la1_2 = new int[] { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0,
