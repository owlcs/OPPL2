...
public abstract class AbstractExpectedErrorCheckerErrorListener implements ErrorListener {
	...
	public void unrecognisedSymbol(CommonTree t) {
		this.getErrorChecker().unexpectedError("Unexpected unrecognised symbol on token " + t);
	}
	...
	public void incompatibleSymbolType(CommonTree t, Type type, CommonTree expression) {
		...
		this.getErrorChecker().unexpectedError(errorMessage);
	}
	...
	public void incompatibleSymbols(CommonTree parentExpression, CommonTree... trees) {
		StringBuilder out = new StringBuilder();
		Formatter formatter = new Formatter(out, Locale.getDefault());
		formatter.format("Unexpected incompatible symbols  inside %s [", parentExpression);
	...
	public void illegalToken(CommonTree t, String message) {
		StringBuilder out = new StringBuilder();
		Formatter formatter = new Formatter(out, Locale.getDefault());
		formatter.format("Unexpected illegal token for token %s type message %s", t, message);
		String errorMessage = out.toString();
		this.getErrorChecker().unexpectedError(errorMessage);
	}
	...
	public static ErrorListener getIncompatibleSymbolType(final ErrorCheckerCallBack errorChecker) {
		return new AbstractExpectedErrorCheckerErrorListener(errorChecker) {
			@Override
			public void incompatibleSymbolType(CommonTree t, Type type, CommonTree expression) {
				errorChecker.getErrorListenerForExpectedError().incompatibleSymbolType(
						t,
						type,
						expression);
			}
	...
	public static ErrorListener getReportThrowable(final ErrorCheckerCallBack errorChecker) {
		return new AbstractExpectedErrorCheckerErrorListener(errorChecker) {
			@Override
			public void reportThrowable(Throwable t, int line, int charPosInLine, int length) {
				errorChecker.getErrorListenerForExpectedError().reportThrowable(
						t,
						line,
						charPosInLine,
						length);
