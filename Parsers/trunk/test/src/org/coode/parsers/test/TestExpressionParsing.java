...
public class TestExpressionParsing extends TestCase {
	...
	protected ManchesterOWLSyntaxTree parse(String input) {
		...
		ManchesterOWLSyntaxParser parser = new ManchesterOWLSyntaxParser(tokens, this.errorListener);
	...
	public void testEscapeIdentifiersAsEntityReferences() {
		ManchesterOWLSyntaxTree parsed = this.parse("Pizza and 'hasTopping' some Thing");
		assertNotNull(parsed);
		System.out.println(parsed.toStringTree());
	}
	...
	public void testDataRestriction() {
		ManchesterOWLSyntaxTree parsed = this.parse("hasKCal some float[ > \"1\", > \"2\"]");
		assertNotNull(parsed);
		System.out.println(parsed.toStringTree());
	}

	public void testDataIntersectionOf() {
		ManchesterOWLSyntaxTree parsed = this.parse("float[ > \"1\"] and int [< \"1\"]");
		assertNotNull(parsed);
		System.out.println(parsed.toStringTree());
	}
}
