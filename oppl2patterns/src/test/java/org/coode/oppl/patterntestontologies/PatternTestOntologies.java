...
public class PatternTestOntologies {
    ...
    public static OWLOntology food(OWLOntologyManager m) {
        ...
        try {
            OWLOntology siblings =
                m.createOntology(IRI("http://www.coode.org/patterns/examples/food"));
            ...
            OWLAnnotationProperty VegetarianCourseFreePatternInstantiation =
                ann(ns1 + "VegetarianCourseFreePatternInstantiation");
            OWLAnnotationProperty VegetarianCoursenullPatternInstantiation =
                ann(ns1 + "VegetarianCoursenullPatternInstantiation");
            OWLAnnotationProperty VegetarianMenuFreeMenuPatternInstantiation =
                ann(ns1 + "VegetarianMenuFreeMenuPatternInstantiation");
            OWLAnnotationProperty createdBy = ann(ns1 + "createdBy");
            OWLLiteral pattern1 = Literal(
                "?x:CLASS,?forbiddenContent:CLASS=createUnion(?x.VALUES) BEGIN ADD $thisClass equivalentTo contains only (not ?forbiddenContent ) END;  A ?x free stuff ; RETURN $thisClass");
            OWLLiteral pattern2 = Literal(
            ...
            declare(siblings, Probe, Free, FreeMenu, Hierarchy,
                VegetarianCourseFreePatternInstantiation, VegetarianCoursenullPatternInstantiation,
                VegetarianMenuFreeMenuPatternInstantiation, createdBy);
            Set<OWLAnnotation> ann1 = Collections.singleton(ann(createdBy,
                Literal("http://www.co-ode.org/patterns#VegetarianCourseFreePatternInstantiation",
                    string())));
            Set<OWLAnnotation> ann2 = Collections.singleton(ann(createdBy,
            ...
            state(siblings, label(Course, "Portata@it"), label(Food, "Cibo@it"),
                label(contains, "contiene@it"), label(Meat, "Carne@it"), label(Menu, "Menù@it"),
                label(Vegetable, "Verdura@it"), label(VegetarianCourse, "Portata_Vegetariana@it"),
                label(VegetarianMenu, "Menù_Vegetariano@it"));
            state(siblings,
                ann(VegetarianCourseFreePatternInstantiation, VegetarianCourse.getIRI(),
                    Literal("$Free(Meat)", string())),
                ann(VegetarianMenuFreeMenuPatternInstantiation, VegetarianMenu.getIRI(),
                    Literal("$FreeMenu(Meat)", string())),
    ...
    public static OWLOntology patternedPizza(OWLOntologyManager m) {
        try {
            ...
            OWLAnnotation ann = ann(named, Literal(
    ...
    public static OWLOntology dul(OWLOntologyManager m) {
        ...
        try {
            OWLOntology _dul = m.createOntology(IRI("http://www.loa-cnr.it/ontologies/DUL.owl"));
            ...
            OWLObjectProperty isConcretelyExpressedBy = object(ns + "isConcretelyExpressedBy");
            ...
            OWLObjectProperty isReferenceOfInformationRealizedBy =
                object(ns + "isReferenceOfInformationRealizedBy");
            OWLObjectProperty isRegionFor = object(ns + "isRegionFor");
            OWLObjectProperty isRelatedToConcept = object(ns + "isRelatedToConcept");
            OWLObjectProperty isRelatedToDescription = object(ns + "isRelatedToDescription");
            ...
            OWLObjectProperty realizesInformationAbout = object(ns + "realizesInformationAbout");
            ...
            OWLDataProperty hasParameterDataValue = DataProperty(ns + "hasParameterDataValue");
            ...
            state(_dul, label(Abstract, "Astratto@it", "Abstract@en"),
                label(Action, "Action@en", "Azione@it"), label(Agent, "Agent@en", "Agente@it"),
                ...
                label(DesignedArtifact, "Artefatto progettato@it", "Designed artifact@en"),
                label(Diagnosis, "Diagnosis@en", "Diagnosi@it"),
                label(Entity, "Entity@en", "Entità@it"), label(Event, "Event@en", "Evento@it"),
                label(hasEventDate, "has event date@en", "evento ha data@it"),
                label(hasIntervalDate, "intervallo ha data@it", "has interval date@en"),
                label(hasParameterDataValue, "ha valore@it", "has parameter data value@en"),
                ...
                label(Goal, "Goal@en", "Scopo@it"), label(Group, "Gruppo@it", "Group@en"),
                label(InformationObject, "Information object@en", "Oggetto informativo@it"),
                label(InformationRealization, "Information realization@en",
                    "Informazione concreta@it"),
                label(LocalConcept, "Local concept@en"), label(Method, "Metodo@it", "Method@en"),
                label(Narrative, "Narrative@en"),
                label(NaturalPerson, "Persona fisica@it", "Natural person@en"),
                label(Norm, "Norma@it", "Norm@en"), label(Object, "Object@en", "Oggetto@it"),
                label(Organization, "Organization@en", "Organizzazione@it"),
                label(Parameter, "Parametro@it"), label(Parameter, "Parameter@en"),
                label(Pattern, "Pattern@en"), label(Person, "Persona {it}"),
                label(Person, "Person@en"), label(Personification, "Personification@en"),
                ...
                label(PhysicalAttribute, "Physical attribute@en", "Caratteristica fisica@it"),
                label(PhysicalObject, "Oggetto fisico@it", "Physical object@en"),
                label(PhysicalPlace, "Luogo fisico@it", "Physical place@en"),
                label(Place, "Place@en", "Luogo@it"), label(PhysicalBody, "Physical body@en"),
                label(Process, "Process@en", "Processo@it"), label(Plan, "Plan@en", "Piano@it"),
                ...
                label(Right, "Right@en", "Diritto@it"), label(Role, "Ruolo@it", "Role@en"),
                ...
                label(SocialObjectAttribute, "Social attribute@en", "Caratteristica sociale@it"),
                label(SocialRelation, "Social relation@en", "Relazione sociale@it"),
                label(SpaceRegion, "Space region@en", "Regione di spazio@it"),
                label(Substance, "Sostanza@it", "Substance@en"), label(Task, "Task@en", "Task@it"),
                ...
                label(TypeCollection, "Type collection@en", "Collezione di un tipo@it"),
                label(UnitOfMeasure, "Unit of measure@en", "Unità di misura@it"),
                label(Workflow, "Workflow@it", "Workflow@en"),
                label(WorkflowExecution, "Esecuzione di workflow@it", "Workflow execution@en"),
                ...
                label(concretelyExpresses, "concretely expresses@en", "esprime concretamente@it"),
                label(coparticipatesWith, "co-participates with@en", "copartecipa con@it"),
                ...
                label(directlyPrecedes, "precede direttamente@it", "directly precedes@en"),
                ...
                label(expressesConcept, "esprime il concetto@it", "expresses concept@en"),
                label(farFrom, "far from@en"), label(follows, "follows@en", "segue@it"),
                ...
                label(isConceptUsedIn, "è un concetto usato in@it", "is concept used in@en"),
                label(isConceptualizedBy, "è concettualizzato da@it", "is conceptualized by@en"),
                ...
                label(isParticipantIn, "is participant in@en", "è un partecipante a@it"),
                label(isPostconditionOf, "is postcondition of@en", "è postcondizione di@it"),
                label(isPreconditionOf, "is precondition of@en", "è precondizione di@it"),
                ...
                label(isParametrizedBy, "is parametrized by@en", "è parametrizzato da@it"),
                ...
                label(isAgentInvolvedIn, "è un agente coinvolto in@it", "is agent involved in@en"),
                label(isCharacterizedBy, "è caratterizzato da@it", "is characterized by {@en-us}"),
                ...
                label(hasPostcondition, "has postcondition@en", "ha postcondizione@it"),
                ...
                label(realizesInformationAbout, "realizza informazione che si riferisce a a@it",
                    "realizes information about@en"),
                label(specializes, "specializza@it", "specializes@en"),
                label(satisfies, "satisfies@en", "soddisfa@it"),
                label(overlaps, "sovrapposto a@it", "overlaps@en"), label(nearTo, "near to@en"),
                label(parametrizes, "parametrizza@it", "parametrizes@en"),
                label(precedes, "precedes@en", "precede@it"),
                label(isRoleDefinedIn, "è un ruolo definito in@it", "is role defined in@en"),
                ...
                label(isSpecializedBy, "è specializzato da@it", "is specialized by@en"),
                label(isTaskDefinedIn, "è un task definito in@it", "is task defined in@en"),
                label(isSubordinatedTo, "is subordinated to@en", "è subordinato a@it"),
                label(isSuperordinatedTo, "è superordinato a@it", "is superordinated to@en"),
                label(isTimeIncludedIn, "is time included in@en", "è un tempo nel contesto di@it"),
                ...
                label(isRelatedToConcept, "è associato al concetto@it", "is related to concept@en"),
                ...
                label(hasRegionDataValue, "has region data value@en", "regione ha valore@it"),
                label(mappableTo, "mappable to@en"));
            state(_dul, sub(Pattern, Relation), sub(Abstract, Entity), disjoint(Abstract, Event),
                disjoint(Abstract, Object), disjoint(Abstract, Quality), sub(Action, Event),
                sub(Agent, Object), sub(Amount, Region), disjoint(Amount, PhysicalAttribute),
                disjoint(Amount, SpaceRegion), disjoint(Amount, TimeInterval),
                sub(BiologicalObject, PhysicalBody), sub(ChemicalObject, PhysicalBody),
                sub(Classification, Situation), sub(Action, some(hasParticipant, Agent)),
                sub(Action, min(1, executesTask)), eq(Agent, or(SocialAgent, PhysicalAgent)),
                ...
                sub(Collection, all(hasPart, Collection)), sub(Collective, all(hasMember, Agent)),
                sub(CollectiveAgent, some(actsThrough, some(isMemberOf, Collective))),
                sub(CollectiveAgent,
                    some(isIntroducedBy, and(some(unifies, Collective), Description))),
                sub(Concept, some(isDefinedIn, Description)), sub(Concept, all(hasPart, Concept)),
                sub(Collection, SocialObject), sub(Collective, Collection),
                sub(CollectiveAgent, SocialAgent), sub(Community, CollectiveAgent),
                sub(Concept, SocialObject), disjoint(Concept, Description),
                disjoint(Concept, InformationObject), disjoint(Concept, Situation),
                disjoint(Concept, SocialAgent), sub(Configuration, Collection),
                sub(Contract, Description), sub(Description, SocialObject),
                disjoint(Description, InformationObject), disjoint(Description, Situation),
                disjoint(Description, SocialAgent), sub(Design, Description),
                sub(DesignedArtifact, PhysicalArtifact), sub(DesignedSubstance, DesignedArtifact),
                sub(DesignedSubstance, FunctionalSubstance), sub(Diagnosis, Description),
                sub(Event, Entity), sub(DesignedArtifact, some(isDescribedBy, Design)),
                eq(Entity, or(Abstract, Event, Object, Quality, Region)),
                sub(Event, some(hasParticipant, Object)), sub(Event, all(hasConstituent, Event)),
                sub(Event, all(hasPart, Event)), sub(EventType, all(classifies, Event)),
                ...
                eq(InformationRealization,
                    and(some(realizes, InformationObject),
                        or(and(some(hasParticipant, PhysicalObject), Event), PhysicalObject))),
                ...
                eq(LocalConcept, and(exact(1, isDefinedIn), Concept)), disjoint(Event, Object),
                disjoint(Event, Quality), sub(EventType, Concept), disjoint(EventType, Parameter),
                disjoint(EventType, Role), sub(FormalEntity, Abstract),
                sub(FunctionalSubstance, Substance), sub(Goal, Description),
                sub(Group, CollectiveAgent), sub(InformationEntity, Entity),
                sub(InformationObject, InformationEntity), sub(InformationObject, SocialObject),
                disjoint(InformationObject, Situation), disjoint(InformationObject, SocialAgent),
                sub(InformationRealization, InformationEntity),
                sub(InformationRealization, InformationEntity), sub(LocalConcept, Concept),
                sub(Method, Description), sub(Narrative, Description), sub(NaturalPerson, Person),
                sub(NaturalPerson, PhysicalAgent), sub(Norm, Description), sub(Object, Entity),
                eq(Object, or(SocialObject, PhysicalObject)),
                sub(Object, some(isParticipantIn, Event)), sub(Object, all(hasConstituent, Object)),
                sub(Object, all(hasPart, Object)), sub(Object, all(isClassifiedBy, Role)),
                sub(Parameter, all(classifies, Region)), sub(Parameter, all(hasPart, Parameter)),
                eq(Person, or(SocialPerson, NaturalPerson)), disjoint(Object, Quality),
                sub(Organism, BiologicalObject), sub(Organization, SocialAgent),
                sub(Parameter, Concept), disjoint(Parameter, Role), sub(Person, Agent),
                sub(Personification, SocialAgent), sub(PhysicalAgent, Agent),
                sub(PhysicalAgent, PhysicalObject), sub(PhysicalArtifact, PhysicalObject),
                sub(PhysicalAttribute, Region), disjoint(PhysicalAttribute, SpaceRegion),
                disjoint(PhysicalAttribute, TimeInterval), sub(PhysicalBody, PhysicalObject),
                sub(PhysicalObject, Object), disjoint(PhysicalObject, SocialObject),
                sub(PhysicalPlace, PhysicalObject), sub(Place, SocialObject),
                sub(Plan, Description), sub(PhysicalAgent, some(conceptualizes, Plan)),
                sub(PhysicalArtifact, some(isDescribedBy, Plan)),
                sub(PhysicalAttribute, all(isRegionFor, PhysicalObject)),
                sub(PhysicalObject, all(hasPart, PhysicalObject)), sub(Place, min(1, isLocationOf)),
                sub(Plan, some(hasComponent, Goal)),
                eq(PlanExecution, and(some(satisfies, Plan), Situation)),
                sub(Project, some(defines, Role)), sub(Project, some(defines, Task)),
                sub(Project, min(3, defines)), sub(Quality, some(isQualityOf, Entity)),
                sub(Quality, all(hasConstituent, Quality)), sub(Quality, all(hasPart, Quality)),
                sub(Region, all(hasConstituent, Region)), sub(Region, all(hasPart, Region)),
                sub(Region, all(overlaps, Region)), sub(Region, all(precedes, Region)),
                sub(Right, min(2, definesRole)), sub(Right, min(1, definesTask)),
                sub(Role, all(classifies, Object)), sub(Role, all(hasPart, Role)),
                sub(Situation, some(satisfies, Description)),
                sub(SocialAgent, some(actsThrough, PhysicalAgent)),
                eq(SocialObject,
                    or(Collection, Concept, Description, InformationObject, Place, Situation,
                        SocialAgent)),
                ...
                sub(Task, all(hasPart, Task)), sub(Task, all(isExecutedIn, Action)),
                sub(Task, all(isTaskDefinedIn, Description)), sub(Task, all(isTaskOf, Role)),
                sub(Theory, some(hasComponent, Relation)),
                sub(Transition, some(isSettingFor, Process)),
                sub(Transition, some(isSettingFor, Situation)),
                sub(Transition, min(2, includesEvent)), sub(Transition, min(1, includesObject)),
                sub(Transition, min(3, includesTime)), sub(Transition, min(8, isSettingFor)),
                sub(UnitOfMeasure, some(parametrizes, Region)), sub(Workflow, some(defines, Role)),
                sub(Workflow, some(defines, Task)), sub(Workflow, min(3, defines)),
                eq(WorkflowExecution, and(some(satisfies, Workflow), PlanExecution)),
                inverse(actsFor, actsThrough), domain(actsFor, Agent), range(actsFor, SocialAgent),
                domain(actsThrough, SocialAgent), range(actsThrough, Agent),
                inverse(characterizes, isCharacterizedBy), domain(characterizes, Concept),
                range(characterizes, Collection), inverse(isClassifiedBy, classifies),
                domain(classifies, Concept), range(classifies, Entity),
                inverse(conceptualizes, isConceptualizedBy), domain(conceptualizes, Agent),
                ...
                inverse(coparticipatesWith, coparticipatesWith), symmetric(coparticipatesWith),
                domain(coparticipatesWith, Object), range(coparticipatesWith, Object),
                inverse(covers, isCoveredBy), domain(covers, Concept), range(covers, Collection),
                sub(defines, usesConcept), inverse(defines, isDefinedIn),
                domain(defines, Description), range(defines, Concept), sub(definesRole, defines),
                inverse(definesRole, isRoleDefinedIn), domain(definesRole, Description),
                range(definesRole, Role), sub(definesTask, defines),
                inverse(definesTask, isTaskDefinedIn), domain(definesTask, Description),
                range(definesTask, Task), inverse(isDescribedBy, describes),
                domain(describes, Description), range(describes, Entity),
                sub(directlyFollows, follows), inverse(directlyFollows, directlyPrecedes),
                sub(directlyPrecedes, precedes), sub(executesTask, isClassifiedBy),
                inverse(executesTask, isExecutedIn), domain(executesTask, Action),
                range(executesTask, Task), sub(expands, isRelatedToDescription),
                inverse(expands, isExpandedIn), domain(expands, Description),
                range(expands, Description), inverse(isExpressedBy, expresses),
                domain(expresses, InformationObject), range(expresses, SocialObject),
                sub(expressesConcept, expresses), inverse(expressesConcept, isConceptExpressedBy),
                domain(expressesConcept, InformationObject), range(expressesConcept, Concept),
                inverse(farFrom, farFrom), symmetric(farFrom), domain(farFrom, Entity),
                range(farFrom, Entity), inverse(precedes, follows), transitive(follows),
                domain(follows, Entity), range(follows, Entity),
                inverse(hasCommonBoundary, hasCommonBoundary), symmetric(hasCommonBoundary),
                domain(hasCommonBoundary, Entity), range(hasCommonBoundary, Entity),
                sub(hasComponent, hasPart), inverse(hasComponent, isComponentOf),
                domain(hasComponent, Object), range(hasComponent, Object),
                inverse(isConstituentOf, hasConstituent), domain(hasConstituent, Entity),
                range(hasConstituent, Entity), sub(hasConstraint, isClassifiedBy),
                inverse(isConstraintFor, hasConstraint), domain(hasConstraint, Entity),
                range(hasConstraint, Parameter), sub(PlanExecution, Situation), sub(Process, Event),
                sub(Project, Plan), sub(Role, Concept), sub(Quality, Entity),
                sub(Relation, Description), sub(Region, Abstract), sub(Right, Description),
                sub(Set, FormalEntity), sub(Situation, SocialObject),
                disjoint(Situation, SocialAgent), sub(SocialAgent, Agent),
                sub(SocialAgent, SocialObject), sub(SocialObject, Object), sub(SpaceRegion, Region),
                sub(Transition, Situation), disjoint(SpaceRegion, TimeInterval),
                sub(SpatioTemporalRegion, Region), sub(SocialObjectAttribute, Region),
                sub(SocialPerson, Person), sub(SocialPerson, SocialAgent),
                sub(SocialRelation, Relation), sub(Substance, PhysicalBody), sub(Task, EventType),
                sub(Theory, Description), sub(TimeInterval, Region), sub(UnitOfMeasure, Parameter),
                sub(WorkflowExecution, PlanExecution), sub(TypeCollection, Collection),
                sub(Workflow, Plan), inverse(isLocationOf, hasLocation),
                domain(hasLocation, Entity), range(hasLocation, Entity),
                inverse(hasMember, isMemberOf), domain(hasMember, Collection),
                range(hasMember, Entity), sub(hasParameter, isRelatedToConcept),
                inverse(hasParameter, isParameterFor), domain(hasParameter, Concept),
                range(hasParameter, Parameter), inverse(isPartOf, hasPart), transitive(hasPart),
                domain(hasPart, Entity), range(hasPart, Entity),
                inverse(hasParticipant, isParticipantIn), domain(hasParticipant, Event),
                range(hasParticipant, Object), sub(hasPostcondition, directlyPrecedes),
                inverse(isPostconditionOf, hasPostcondition), domain(hasPostcondition, Situation),
                range(hasPostcondition, Situation), sub(hasPrecondition, directlyFollows),
                inverse(isPreconditionOf, hasPrecondition), domain(hasPrecondition, Situation),
                range(hasPrecondition, Situation), inverse(isQualityOf, hasQuality),
                domain(hasQuality, Entity), range(hasQuality, Quality),
                inverse(hasRegion, isRegionFor), domain(hasRegion, Entity),
                range(hasRegion, Region), sub(hasRole, isClassifiedBy), inverse(isRoleOf, hasRole),
                domain(hasRole, Object), range(hasRole, Role), inverse(hasSetting, isSettingFor),
                domain(hasSetting, Entity), range(hasSetting, Situation),
                sub(hasTask, isRelatedToConcept), inverse(hasTask, isTaskOf), domain(hasTask, Role),
                range(hasTask, Task), sub(includesAction, includesEvent),
                inverse(includesAction, isActionIncludedIn), domain(includesAction, Situation),
                range(includesAction, Action), sub(includesAgent, includesObject),
                inverse(isAgentIncludedIn, includesAgent), domain(includesAgent, Situation),
                range(includesAgent, Agent), sub(includesEvent, isSettingFor),
                inverse(isEventIncludedIn, includesEvent), domain(includesEvent, Situation),
                range(includesEvent, Event), sub(includesObject, isSettingFor),
                inverse(isObjectIncludedIn, includesObject), domain(includesObject, Situation),
                range(includesObject, Object), sub(includesTime, isSettingFor),
                inverse(includesTime, isTimeIncludedIn), domain(includesTime, Situation),
                range(includesTime, TimeInterval), inverse(introduces, isIntroducedBy),
                domain(introduces, Description), range(introduces, SocialAgent),
                sub(involvesAgent, hasParticipant), inverse(isAgentInvolvedIn, involvesAgent),
                domain(involvesAgent, Event), range(involvesAgent, Agent),
                inverse(isAbout, isReferenceOf), domain(isAbout, InformationObject),
                range(isAbout, Entity), sub(isActionIncludedIn, isEventIncludedIn),
                domain(isActionIncludedIn, Action), range(isActionIncludedIn, Situation),
                sub(isAgentIncludedIn, isObjectIncludedIn), domain(isAgentIncludedIn, Agent),
                range(isAgentIncludedIn, Situation), sub(isAgentInvolvedIn, isParticipantIn),
                domain(isAgentInvolvedIn, Agent), range(isAgentInvolvedIn, Event),
                domain(isCharacterizedBy, Collection), range(isCharacterizedBy, Concept),
                domain(isClassifiedBy, Entity), range(isClassifiedBy, Concept),
                sub(isComponentOf, isPartOf), domain(isComponentOf, Object),
                range(isComponentOf, Object), sub(isConceptExpressedBy, isExpressedBy),
                domain(isConceptExpressedBy, Concept),
                range(isConceptExpressedBy, InformationObject),
                inverse(usesConcept, isConceptUsedIn), domain(isConceptUsedIn, Concept),
                range(isConceptUsedIn, Description), domain(isConceptualizedBy, SocialObject),
                range(isConceptualizedBy, Agent), domain(isConcretelyExpressedBy, SocialObject),
                range(isConcretelyExpressedBy, InformationRealization),
                domain(isConstituentOf, Entity), range(isConstituentOf, Entity),
                sub(isConstraintFor, classifies), domain(isConstraintFor, Parameter),
                range(isConstraintFor, Entity), domain(isCoveredBy, Collection),
                range(isCoveredBy, Concept), sub(isDefinedIn, isConceptUsedIn),
                domain(isDefinedIn, Concept), range(isDefinedIn, Description),
                domain(isDescribedBy, Entity), range(isDescribedBy, Description),
                sub(isEventIncludedIn, hasSetting), domain(isEventIncludedIn, Event),
                range(isEventIncludedIn, Situation), sub(isExecutedIn, classifies),
                domain(isExecutedIn, Task), range(isExecutedIn, Action),
                sub(isExpandedIn, isRelatedToDescription), domain(isExpandedIn, Description),
                range(isExpandedIn, Description), domain(isExpressedBy, SocialObject),
                range(isExpressedBy, InformationObject), domain(isIntroducedBy, SocialAgent),
                range(isIntroducedBy, Description), domain(isLocationOf, Entity),
                range(isLocationOf, Entity), domain(isMemberOf, Entity),
                range(isMemberOf, Collection), sub(isObjectIncludedIn, hasSetting),
                domain(isObjectIncludedIn, Object), range(isObjectIncludedIn, Situation),
                sub(isObservableAt, hasRegion), inverse(isTimeOfObservationOf, isObservableAt),
                domain(isObservableAt, Entity), range(isObservableAt, TimeInterval),
                sub(isParameterFor, isRelatedToConcept), domain(isParameterFor, Parameter),
                range(isParameterFor, Concept), sub(isParametrizedBy, isClassifiedBy),
                inverse(isParametrizedBy, parametrizes), domain(isParametrizedBy, Region),
                range(isParametrizedBy, Parameter), transitive(isPartOf), domain(isPartOf, Entity),
                range(isPartOf, Entity), domain(isParticipantIn, Object),
                range(isParticipantIn, Event), sub(isPostconditionOf, directlyFollows),
                domain(isPostconditionOf, Situation), range(isPostconditionOf, Situation),
                sub(isPreconditionOf, directlyPrecedes), domain(isPreconditionOf, Situation),
                range(isPreconditionOf, Situation), domain(isQualityOf, Quality),
                range(isQualityOf, Entity), inverse(realizes, isRealizedBy),
                domain(isRealizedBy, InformationObject),
                range(isRealizedBy, InformationRealization), domain(isReferenceOf, Entity),
                ...
                inverse(isRelatedToConcept, isRelatedToConcept), symmetric(isRelatedToConcept),
                domain(isRelatedToConcept, Concept), range(isRelatedToConcept, Concept),
                inverse(isRelatedToDescription, isRelatedToDescription),
                symmetric(isRelatedToDescription), domain(isRelatedToDescription, Description),
                range(isRelatedToDescription, Description), sub(isRoleDefinedIn, isDefinedIn),
                domain(isRoleDefinedIn, Role), range(isRoleDefinedIn, Description),
                sub(isRoleOf, classifies), domain(isRoleOf, Role), range(isRoleOf, Object),
                inverse(isSatisfiedBy, satisfies), domain(isSatisfiedBy, Description),
                range(isSatisfiedBy, Situation), domain(isSettingFor, Situation),
                range(isSettingFor, Entity), inverse(specializes, isSpecializedBy),
                transitive(isSpecializedBy), domain(isSpecializedBy, SocialObject),
                range(isSpecializedBy, SocialObject), sub(isSubordinatedTo, directlyFollows),
                sub(isSubordinatedTo, isRelatedToConcept),
                inverse(isSuperordinatedTo, isSubordinatedTo), domain(isSubordinatedTo, Concept),
                range(isSubordinatedTo, Concept), sub(isSuperordinatedTo, directlyPrecedes),
                sub(isSuperordinatedTo, isRelatedToConcept), domain(isSuperordinatedTo, Concept),
                range(isSuperordinatedTo, Concept), sub(isTaskDefinedIn, isDefinedIn),
                domain(isTaskDefinedIn, Task), range(isTaskDefinedIn, Description),
                sub(isTaskOf, isRelatedToConcept), domain(isTaskOf, Task), range(isTaskOf, Role),
                sub(isTimeIncludedIn, hasSetting), domain(isTimeIncludedIn, TimeInterval),
                range(isTimeIncludedIn, Situation), sub(isTimeOfObservationOf, isRegionFor),
                domain(isTimeOfObservationOf, TimeInterval), range(isTimeOfObservationOf, Entity),
                inverse(isUnifiedBy, unifies), domain(isUnifiedBy, Collection),
                range(isUnifiedBy, Description), inverse(nearTo, nearTo), symmetric(nearTo),
                domain(nearTo, Entity), range(nearTo, Entity), inverse(overlaps, overlaps),
                symmetric(overlaps), domain(overlaps, Entity), range(overlaps, Entity),
                sub(parametrizes, classifies), domain(parametrizes, Parameter),
                range(parametrizes, Region), transitive(precedes), domain(precedes, Entity),
                ...
                range(realizesInformationAbout, Entity), domain(satisfies, Situation),
                range(satisfies, Description), transitive(specializes),
                domain(specializes, SocialObject), range(specializes, SocialObject),
                domain(unifies, Description), range(unifies, Collection),
                domain(usesConcept, Description), range(usesConcept, Concept),
                domain(hasDataValue, Entity), sub(hasEventDate, hasDataValue),
                domain(hasEventDate, Event), range(hasEventDate, date),
                sub(hasIntervalDate, hasRegionDataValue), domain(hasIntervalDate, TimeInterval),
                range(hasIntervalDate, date), sub(hasParameterDataValue, hasDataValue),
    ...
    public static OWLOntology patternedDul(OWLOntologyManager m) {
        try {
            ...
            String lit1 =
                "?informationObject:CLASS, ?informationRealization:CLASS,\n\n ?realizationProperty:OBJECTPROPERTY BEGIN ADD"
            ...
            OWLAnnotation ann1 = ann(ann("http://www.co-ode.org/patterns#informationRealization"),
                Literal(lit1, string()));
            OWLAnnotation ann2 = ann(ann("http://www.co-ode.org/patterns#timeIndexedPersonRole"),
                Literal(lit2, string()));
            m.applyChange(new AddOntologyAnnotation(siblings, ann2));
            m.applyChange(new AddOntologyAnnotation(siblings, ann1));
            declare(siblings, ann("http://www.co-ode.org/patterns#informationRealization"),
    ...
    private static final OWLDatatype any =
        dataFactory.getOWLDatatype(XSDVocabulary.ANY_URI.getIRI());
    private static final OWLDatatype date = dataFactory.getOWLDatatype(XSDVocabulary.DATE.getIRI());
    ...
    private static OWLEquivalentClassesAxiom eq(Set<OWLAnnotation> a,
        ...
        return dataFactory.getOWLEquivalentClassesAxiom(Arrays.asList(classExpressions), a);
    }
    ...
    private static OWLSymmetricObjectPropertyAxiom symmetric(OWLObjectPropertyExpression property) {
    ...
    private static OWLAnnotation ann(OWLAnnotationProperty property, OWLAnnotationValue value) {
