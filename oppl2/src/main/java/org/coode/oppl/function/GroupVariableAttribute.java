    ...
    public GroupVariableAttribute(RegexpGeneratedVariable<O> variable, int index) {
        ...
        if (index < 0) {
            throw new IllegalArgumentException(
                String.format("Invalid index %d", Integer.valueOf(index)));
        }
        this.index = index;
    ...
    public ValueComputation<String> getValueComputation(
        ...
        return opplFunction -> {
            ...
            OWLObject assignmentValue = parameters.getBindingNode()
                .getAssignmentValue(GroupVariableAttribute.this.getVariable(), parameters);
            if (assignmentValue != null) {
                ManchesterSyntaxRenderer renderer = parameters.getConstraintSystem()
                    .getOPPLFactory().getManchesterSyntaxRenderer(parameters.getConstraintSystem());
                assignmentValue.accept(renderer);
                Matcher matcher = GroupVariableAttribute.this.getRegexpGeneratedVariable()
