 ...
 */
public final class OWLClassSubClassOfAxiomSolvability
            ...
            public SolvabilitySearchNode visit(OWLSubClassOfAxiom axiom) {
                ...
                boolean isSolvable = variableExtractor.extractVariables(axiom).size() == 1
                    && (variableExtractor.extractVariables(axiom.getSuperClass()).isEmpty()
                        ...
                        && variableExtractor.extractVariables(axiom.getSubClass()).size() == 1
                        || variableExtractor.extractVariables(axiom.getSubClass()).isEmpty()
                            && !axiom.getSuperClass().isAnonymous() && variableExtractor
                                .extractVariables(axiom.getSuperClass()).size() == 1);
                if (isSolvable) {
                    Variable<?> variable =
                        variableExtractor.extractVariables(axiom).iterator().next();
                    toReturn = variableExtractor.extractVariables(axiom.getSubClass()).isEmpty()
                        ? new SolvableSearchNode(variable, axiom, bindingNode,
                            OWLClassSubClassOfAxiomSolvability.this.getQuerySolver()
                                .getSuperClasses(axiom.getSubClass()))
                        : new SolvableSearchNode(variable, axiom, bindingNode,
                            OWLClassSubClassOfAxiomSolvability.this.getQuerySolver()
                                .getSubClasses(axiom.getSuperClass()));
                }
                return toReturn;
            }
        });
