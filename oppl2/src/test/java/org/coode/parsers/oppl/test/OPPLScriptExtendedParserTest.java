...
public class OPPLScriptExtendedParserTest {
    ...
    public void shouldTestOld2() {
        tester(
            "?y:CLASS, ?k:CONSTANT = create(\"mytest\"), ?x:CLASS = create(\"Test\"+?k.RENDERING) SELECT ?y SubClassOf Pizza\n"
                + " BEGIN ADD ?x SubClassOf ?y END;", pizza);
    }
    ...
    public void shouldTestOld15() {
        tester(
            "?regexp:CLASS= MATCH (\"(Piz)za\"), ?x:CLASS = create(?regexp.GROUPS(1)) SELECT ?regexp SubClassOf DomainConcept\n"
