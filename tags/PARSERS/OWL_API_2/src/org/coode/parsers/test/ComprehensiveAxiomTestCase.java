...
public class ComprehensiveAxiomTestCase extends TestCase {
	...
	public void testSNOMEDFragment() {
		try {
			URI uri = this.getClass().getResource("testLongLabels.owl").toURI();
			OWLOntology ontology = ONTOLOGY_MANAGER.loadOntology(uri);
			AnnotationValueShortFormProvider labelShortFormProvider = new AnnotationValueShortFormProvider(
					Arrays
							.asList(URI
									.create("http://www.w3.org/2000/01/rdf-schema#label")),
					Collections.<URI, List<String>> emptyMap(),
					ONTOLOGY_MANAGER);
			BidirectionalShortFormProviderAdapter shortFormProvider = new BidirectionalShortFormProviderAdapter(
					ONTOLOGY_MANAGER, Collections.singleton(ontology),
					labelShortFormProvider);
			...
			ShortFormEntityRenderer entityRenderer = new ShortFormEntityRenderer(
					labelShortFormProvider);
			EntityFinder entityFinder = new EntityFinderImpl(ONTOLOGY_MANAGER,
					new OWLEntityRenderingCacheImpl(ONTOLOGY_MANAGER,
							entityRenderer), false);
			this.symtab = new SymbolTable(new OWLEntityCheckerScope(
					entityChecker, entityFinder, entityRenderer),
					ONTOLOGY_MANAGER.getOWLDataFactory());
			ManchesterOWLSyntaxTree parsed = this
					.parse("'Structure of femoral artery (body structure)' subClassOf Thing");
			Type parseType = parsed.getEvalType();
			assertTrue("parse type " + parseType,
					parseType == OWLAxiomType.SUBCLASS);
			assertTrue(parsed.getOWLObject().accept(ASSOCOATION) == OWLAxiomType.SUBCLASS);
			ONTOLOGY_MANAGER.removeOntology(uri);
		} catch (URISyntaxException e) {
			e.printStackTrace();
			fail("Could not load the ontology as there were problems with the URI "
					+ e.getMessage());
		} catch (OWLOntologyCreationException e) {
			e.printStackTrace();
			fail("Could not load the ontology  " + e.getMessage());
		}
	}
}
