...
public class OPPLScriptExtendedParserTest extends OPPLScriptTypesParserTest {
	...
	public void testOld8() {
		this.tester("SELECT America InstanceOf Pais BEGIN REMOVE America InstanceOf Pizza END;");
	}
	...
	public void testOld12() {
		this.tester("SELECT America InstanceOf Pais BEGIN ADD America InstanceOf Thing END;");
	}
	...
	public void testOld21() {
		this.tester("?x:CLASS, ?y:CLASS=MATCH(\"[Piza]+\"+?x.RENDERING) SELECT ?y subClassOf Food, ?x subClassOf Thing WHERE ?y MATCH(\"[Piza]+\"+?x.RENDERING) BEGIN ADD ?y subClassOf Thing END;");
	}
