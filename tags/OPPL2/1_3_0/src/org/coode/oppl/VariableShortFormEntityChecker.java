...
public class VariableShortFormEntityChecker implements OWLEntityChecker {
	private static class CreatedAssignementTreeNode {
			...
			while (!allLeaves) {
				for (CreatedAssignementTreeNode generatedChild : new HashSet<CreatedAssignementTreeNode>(
					...
					if (!generatedChild.isLeaf()) {
						...
						Set<CreatedAssignementTreeNode> generatedChildren = this.generateChildren(generatedChild);
		...
		private Set<CreatedAssignementTreeNode> generateChildren(CreatedAssignementTreeNode node) {
				...
				for (String element : unassignedElements) {
					for (VariableType owlObject : VariableType.values()) {
						...
						Set<CreationAssignemnt> childAssignements = new HashSet<CreationAssignemnt>(
								node.assignments);
						childAssignements.add(new CreationAssignemnt(element, owlObject));
						toReturn.add(new CreatedAssignementTreeNode(childUnassignedElements,
								childAssignements));
					}
				}
			} else {
	 ...
	 */
	public VariableShortFormEntityChecker(OWLEntityChecker simpleOWLEntityChecker,
			ConstraintSystem constraintSystems) {
		this.simpleOWLEntityChecker = simpleOWLEntityChecker;
		this.constraintSystem = constraintSystems;
	}
		...
		if (name.startsWith("!")) {
			...
			if (createdEntity != null && createdEntity instanceof OWLClass) {
				...
				if (creationAssignemnt != null
						&& creationAssignemnt.getType().equals(VariableType.CLASS)) {
					try {
						toReturn = (OWLClass) this.create(entityName, VariableType.CLASS).getOWLEntity();
			...
			if (toReturn == null) {
				...
				if (variable != null && variable.getType().equals(VariableType.CLASS)) {
					toReturn = this.getConstraintSystem().getOPPLFactory().getOWLDataFactory().getOWLClass(
							variable.getURI());
				}
			}
		}
	...
	public OWLDataProperty getOWLDataProperty(String name) {
		...
		if (name.startsWith("!")) {
			String entityName = name.substring(name.indexOf("!") + 1);
			OWLEntity createdEntity = this.created.get(entityName);
			if (createdEntity != null && createdEntity instanceof OWLDataProperty) {
				toReturn = (OWLDataProperty) createdEntity;
			} else if (createdEntity == null) {
				CreationAssignemnt creationAssignemnt = this.fetch(name);
				if (creationAssignemnt != null
						&& creationAssignemnt.getType().equals(VariableType.DATAPROPERTY)) {
					try {
						toReturn = (OWLDataProperty) this.create(
								entityName,
			...
			if (toReturn == null) {
				Variable variable = this.constraintSystem.getVariable(name);
				if (variable != null && variable.getType().equals(VariableType.DATAPROPERTY)) {
					toReturn = this.getConstraintSystem().getOPPLFactory().getOWLDataFactory().getOWLDataProperty(
							variable.getURI());
				}
			}
		}
	...
	public OWLObjectProperty getOWLObjectProperty(String name) {
		...
		if (name.startsWith("!")) {
			String entityName = name.substring(name.indexOf("!") + 1);
			OWLEntity createdEntity = this.created.get(entityName);
			if (createdEntity != null && createdEntity instanceof OWLObjectProperty) {
				toReturn = (OWLObjectProperty) createdEntity;
			} else if (createdEntity == null) {
				CreationAssignemnt creationAssignemnt = this.fetch(name);
				if (creationAssignemnt != null
						&& creationAssignemnt.getType().equals(VariableType.OBJECTPROPERTY)) {
					try {
						toReturn = (OWLObjectProperty) this.create(
								entityName,
			...
			if (toReturn == null) {
				Variable variable = this.constraintSystem.getVariable(name);
				if (variable != null && variable.getType().equals(VariableType.OBJECTPROPERTY)) {
					toReturn = this.getConstraintSystem().getOPPLFactory().getOWLDataFactory().getOWLObjectProperty(
							variable.getURI());
				}
			}
		}
	...
	public OWLIndividual getOWLIndividual(String name) {
		...
		if (name.startsWith("!")) {
			...
			if (createdEntity != null && createdEntity instanceof OWLIndividual) {
				...
				CreationAssignemnt creationAssignemnt = this.fetch(name);
				if (creationAssignemnt != null
						&& creationAssignemnt.getType().equals(VariableType.INDIVIDUAL)) {
					try {
						toReturn = (OWLIndividual) this.create(entityName, VariableType.INDIVIDUAL).getOWLEntity();
					} catch (OWLEntityCreationException e) {
						e.printStackTrace();
					}
				}
			...
			if (toReturn == null) {
				Variable variable = this.constraintSystem.getVariable(name);
				if (variable != null && variable.getType().equals(VariableType.INDIVIDUAL)) {
					toReturn = this.getConstraintSystem().getOPPLFactory().getOWLDataFactory().getOWLIndividual(
							variable.getURI());
				}
			}
		}
	...
	public void applyAdditions() {
		try {
			if (this.lastAdditions != null && !this.lastAdditions.isEmpty()) {
				this.getConstraintSystem().getOntologyManager().applyChanges(this.lastAdditions);
			}
	...
	public CreationAssignemnt fetch(String name) {
		Iterator<CreationAssignemnt> it = this.assignementQueue.element().iterator();
		boolean found = false;
	...
	private OWLEntityCreationSet<? extends OWLEntity> create(String shortName, VariableType type)
		...
		if (clazz != null) {
			org.coode.oppl.entity.OWLEntityFactory owlEntityFactory = this.getConstraintSystem().getOPPLFactory().getOWLEntityFactory();
	...
	public void generate(Set<String> createdElements) {
		CreatedAssignementTreeNode root = new CreatedAssignementTreeNode(createdElements,
				new HashSet<CreationAssignemnt>());
