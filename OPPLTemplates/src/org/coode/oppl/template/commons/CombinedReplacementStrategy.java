...
public class CombinedReplacementStrategy<I, P extends I> implements
	...
	public CombinedReplacementStrategy(
			List<ReplacementStrategy<I, P>> strategies) {
		if (strategies == null) {
	 ...
	 *      org.coode.oppl.template.ParsingStrategy)
	 */
	public final P replace(I input) {
		P replaced = null;
		Iterator<ReplacementStrategy<I, P>> iterator = this.getStrategies()
				.iterator();
		// There is at least one strategy.
		do {
			ReplacementStrategy<I, P> strategy = iterator.next();
			replaced = strategy.replace(replaced);
